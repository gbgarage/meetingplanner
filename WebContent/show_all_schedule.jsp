...
<html>
  ...
  <style type="text/css">
     .cfobject { background-color: #8cb5c0; }
     .mtobject { background-color: #c3deb7; }
     .nlobject { background-color: #ffeebc; }
  </style>
...
<script type="text/javascript">
	  ...
	  if (textStatus=="success") {
		  ...
		  for (var i=0;i<data.length;i++) {
			  ...
			  for (var j=0;j<timeslot.length;j++) {
				  ...
				  if (td1.innerHTML != "" && timeslot[j].substring(0,1)=="T") {
					  td1.setAttribute("class", "mtobject"); 
					  td1.className ="mtobject";
					  //td1.class ="mtobject";
				  }
				  
				  tr1.appendChild(td1);  
			  }
			  datagrid1.appendChild(tr1);
		  }
		  
		  //set up mtobject 
		  	$(".mtobject" ).draggable({ opacity: 0.35, revert: true, revertDuration: 100 });
		  	$(".mtobject").droppable({drop:dropHandler});
	  }
  }
  ...
  function getListConflicts(data, textStatus, jqXHR) {
	  var datagrid2 = document.getElementById("datagrid2");
	  if (textStatus=="success") {
		  //console.log(data);
		  for (var i=0;i<data.length;i++) {
			  var tr1 = document.createElement("tr");
			  ...
			  for (var j=0;j<conflictList.length;j++) {
				  ...
				  if (td1.innerHTML != "" && j==0) {
					  td1.setAttribute("class", "cfobject"); 
					  td1.className ="cfobject";
				  }
			  ...
			  datagrid2.appendChild(tr1);
		  }
		  
		  //set up mtobject 
		  	$(".cfobject" ).draggable({ opacity: 0.35, revert: true, revertDuration: 100 });
		  	$(".cfobject").droppable({drop:dropHandler});
	  }
  }
		...
		<table width=100%>
		   <tr>
		      <td width=50%>
		         ...
		         <table id="datagrid2"  class="mini-grid-table" style="border:1px solid #cccccc;width:100%;overflow:scroll">
		            <tr>
